...
const showInputError = (showFormElement, showInputElement, errorMassage) => {
  const errorElement = showFormElement.querySelector(
    `.${showInputElement.id}-error`
  );
...
const removeInputError = (removeFormElement, removeInputElement) => {
  const errorElement = removeFormElement.querySelector(
    `.${removeInputElement.id}-error`
  );
...
const checkInputValidity = (validityFormElement, validiyInputElement) => {
  if (!validiyInputElement.validity.valid) {
    showInputError(
      validityFormElement,
      validiyInputElement,
      validiyInputElement.validationMessage
    );
  } else {
...
const setEventListener = (formElementInput) => {
  const listInput = Array.from(
    formElementInput.querySelectorAll(".form__info")
  );
